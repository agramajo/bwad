...
contract BWAD is Context, IERC20, Ownable, ReentrancyGuard {
	...
	function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
		(uint256 tTransferAmount, uint256 tHODLrFee, uint256 tDevelopment) = _getTValues(tAmount);
		(uint256 rAmount, uint256 rTransferAmount, uint256 rHODLrFee) = _getRValues(tAmount, tHODLrFee, tDevelopment, _getRate());
		return (rAmount, rTransferAmount, rHODLrFee, tTransferAmount, tHODLrFee, tDevelopment);
	}

	function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
		uint256 tHODLrFee = calculateRewardFee(tAmount);
		uint256 tDevelopment = calculateDevelopmentFee(tAmount);
		uint256 tTransferAmount = tAmount.sub(tHODLrFee).sub(tDevelopment);
		return (tTransferAmount, tHODLrFee, tDevelopment);
	}

	function _getRValues(uint256 tAmount, uint256 tHODLrFee, uint256 tDevelopment, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
		uint256 rAmount = tAmount.mul(currentRate);
		uint256 rHODLrFee = tHODLrFee.mul(currentRate);
		uint256 rDevelopment = tDevelopment.mul(currentRate);
		uint256 rTransferAmount = rAmount.sub(rHODLrFee).sub(rDevelopment);
	...
	function removeAllFee() private {
		if(_rewardFee == 0 && _developmentFee == 0) return;		
